...
export function Graph({
  ...
  initialView?: "map" | "work" | "tasks";
  create?: (kind: "task" | "duty" | "handoff") => void;
  tasksSupporting?: ReactNode;
  onClientBrief?: () => void;
  taskFilter?: string;
  taskQuery?: string;
}) {
  ...
  const [taskFilterValue, setTaskFilterValue] = useState(taskFilter);
  const [taskQueryValue, setTaskQueryValue] = useState(taskQuery);
  const lastInitialView = useRef(initialView);
  useEffect(() => {
    if (lastInitialView.current === initialView) return;
    lastInitialView.current = initialView;
    setView(initialView);
    setSelected(null);
    setList(false);
  }, [initialView]);
  useEffect(() => setTaskFilterValue(taskFilter), [taskFilter]);
  useEffect(() => setTaskQueryValue(taskQuery), [taskQuery]);
  const workflows = records.filter(
    (r) => r.kind === "workflow" && r.state !== "withdrawn",
  );
  const relatedWork = records.filter(
    (record) =>
      ["duty", "handoff"].includes(record.kind) &&
      !["withdrawn", "retracted", "superseded"].includes(record.state),
  );
  ...
  if (!records.length && view === "work")
    return (
      <Empty
        ...
        action={<Button onClick={() => setView("tasks")}>Open Tasks</Button>}
      />
    );
  return (
    <div className={expanded ? "graph-expanded" : "graph-standard"}>
      <div className="toolbar">
        ...
        <div className="actions">
          {onClientBrief && (
            <button className="text-link" onClick={onClientBrief}>
              Client brief →
            </button>
          )}
          {view !== "tasks" && (
      ...
      {view !== "map" &&
        (view === "tasks" ? (
          <section className="graph-tasks">
            <div className="graph-tasks-heading">
              <h2>Tasks</h2>
            </div>
            {create && (
              <div className="graph-task-actions" aria-label="Add work records">
            ...
            <TaskCards
              ...
              query={taskQueryValue}
              open={open}
              primaryControls={
                <div className="task-primary-filters" aria-label="Filter tasks">
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setTaskQueryValue(event.target.value)
                      }
            ...
            {relatedWork.length > 0 && (
              <details className="graph-task-support">
                <summary>
                  Related duties and handoffs ({relatedWork.length})
                </summary>
                <div>
                  {create && (
                    <div className="graph-related-actions">
                      <Button onClick={() => create("duty")}>Add duty</Button>
                      <Button onClick={() => create("handoff")}>
                        Add handoff
                      </Button>
                    </div>
                  )}
                  {relatedWork.map((record) => (
                    <button
                      className="text-link"
                      key={record.id}
                      onClick={() => open(record)}
          ...
          <Empty
            ...
            action={
              <Button onClick={() => setView("tasks")}>Open Tasks</Button>
            }
          />
        ) : (
          <div className="graph-shell">
              ...
              >
                <defs>
                  ...
                  >
                    <path
                      d="M0,0 L8,4 L0,8 Z"
                      className="arrow-head highlight"
                    />
                  </marker>
                </defs>
                <rect
                ...
                {view === "connected" &&
                  [0, 1, 2]
                    .filter(
                      (col) => !layout.nodes.some((n) => n.column === col),
                    )
                    .map((col) => (
                      <text
                        key={`missing-${col}`}
            ...
            {current && (
              <aside className="graph-inspector">
                ...
                {[
                  ...
                  .map((e: any, i: number) => {
                    const target = records.find(
                      (r) =>
                        r.id ===
                        (e.source === current.id ? e.target : e.source),
                    );
                    return (
                      target && (
                        <button
                ...
                {!focus &&
                  ["person", "duty", "task"].includes(current.kind) && (
      ...
      {view === "work" && !list && <p className="subtle">{layout.note}</p>}
    </div>
  );
}
