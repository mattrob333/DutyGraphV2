...
export function previewRoster(csv: string, existing: any[] = []) {
  ...
  const rows = parsed.data.slice(0, 500).map((r, index) => {
    ...
    const issues: string[] = [];
    if (
      [data.name, data.role, data.team, data.externalId].some(
        (v) => v.length > 200,
      ) ||
      [data.email, data.managerEmail].some((v) => v.length > 254)
    )
      issues.push("A roster field exceeds its allowed length.");
    if (!data.name || !data.role || !data.team)
      issues.push("Name, role, and team are required.");
    if (!z.email().safeParse(data.email).success) issues.push("Invalid email.");
