...
Implementation `e4aedfb` passed 300 local tests and [CI 34237031178](https://github.com/mattrob333/DutyGraphV2/actions/runs/34237031178), and deployed READY after migration 0012. The hosted app serves the new work-gap controls, private response flow and stage evidence help; live CSS matches the verified local build. Browser checks used fictional people and injected providers. Real inbox delivery and provider-output quality remain separate checks. See [verification](docs/verification/2026-09-08-work-gap-followups.md).

Business stages are broader than duties: a stage groups work across roles; a duty is an ongoing responsibility; a task is a specific piece of work. There is no universal stage count. Templates are suggested starting points, and saved streams support 1-16 stages. Hover a stage or focus/tap its info button for an explanation and typical work. The popup stays open while the pointer moves into it and fits above or below the button. Company descriptions take precedence; renamed/custom stages ask for a description instead of inheriting unrelated examples. Edit descriptions in Discovery > Edit the profile and stages.

The default Company Work Map uses a compact stage ribbon above a reporting chart that fits the available desktop height. Selection uses a single border and subtle fill; stage changes preserve the chart position. Select a person for scoped duties/tasks. **Fit team** restores the whole chart; **Expand** adds room. The unassigned-work control leads to manual or AI assignment. Light and dark themes, mobile scrolling and the existing graph views remain available. Task cards uses the same saved stages; tasks inherit duty links unless given their own. See [the stage work map](docs/stage-work-map.md).
