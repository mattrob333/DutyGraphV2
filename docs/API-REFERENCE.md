...
Release 0.3. This inventory is generated from server route declarations. It describes 105 implemented method/path declarations, not the larger production target. The report format parameter accepts preview or download. Unknown API routes return a structured 404 from the running server.
...
| GET | /api/v1/companies/{companyId}/audit-brief | server/audit-brief.ts |
| POST | /api/v1/companies/{companyId}/audit-brief/prepare | server/audit-brief.ts |
