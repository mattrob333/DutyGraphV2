  ...
  async function access(
    ...
    return tx(first.rows[0].tenant_id, async (db) => {
      ...
      if (voice) {
        ...
        const count = (
          await db.query(
            "SELECT count(*) FROM audit_events WHERE tenant_id=$1 AND type IN ('kickoff.transcription_requested','team.transcription_requested') AND created_at>now()-interval '24 hours'",
            [invitation.tenant_id],
          )
        ).rows[0].count;
        const perRequest = (
          await db.query(
            "SELECT count(*) FROM audit_events WHERE record_id=$1 AND type IN ('kickoff.transcription_requested','team.transcription_requested') AND created_at>now()-interval '24 hours'",
            [r.id],
          )
        ).rows[0].count;
