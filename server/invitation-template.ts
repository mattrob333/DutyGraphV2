...
export function invitationTemplate({
  ...
  const how = (
    work
      ? [
          "Open your questions. No account or password is needed.",
          "Read the questions and describe a recent example in your own words. Type your answer or use the voice option when available.",
          "Check your answer, then send it to your advisor. Say what you do not know or what needs to be checked.",
        ]
      : contact
        ? [
  ...
  const security = `Your private link expires in 7 days. ${work || contact ? "No account or password is needed." : "Sign in to open your assigned request."} Do not forward this link. Your assigned advisor will review your response.`;
  ...
  const bodyHtml = paragraphs
    ...
    .join("");
  const action = `<a href="${escape(url)}" style="display:inline-block;padding:16px 24px;background:#242829;border-radius:6px;color:white;font-weight:bold;text-decoration:none">${contact ? "Prepare our executive kickoff" : work ? "Open my questions" : "Open your private response page"} →</a>`;
...
${work ? `<div style="padding:20px 24px;margin:24px 0;background:#edf3ef;border-radius:8px"><strong style="font-size:18px">Your questions are ready</strong><p style="margin:8px 0 0">The questions are on your private page. Focus on what starts your work, the steps you take, and who needs the result.</p></div>` : `<h2 style="font-size:21px;margin:32px 0 14px">Questions to consider</h2><p style="color:#656c6d">You can answer these on your private response page.</p><ol style="padding-left:24px">${questions.map((q: string) => `<li style="margin:0 0 18px;padding-left:6px">${escape(q)}</li>`).join("")}</ol>`}
<h2 style="font-size:21px;margin:32px 0 18px;padding-top:22px;border-top:1px solid #deddd7">${contact ? "What to prepare" : "How to respond"}</h2>${how
    ...
    .map(
      (step, i) =>
        `<div style="padding:16px 20px;margin:12px 0;background:#f6f6f3;border-left:3px solid #94b8a4"><strong>${contact ? ["1. Add your team", "2. Choose attendees", "3. Share leadership context", "4. Review and submit"][i] : work ? ["1. Open your questions", "2. Tell us about your work", "3. Check and send"][i] : `Step ${i + 1}`}</strong><p style="margin:8px 0 0">${escape(step)}</p></div>`,
    )
    .join("")}
